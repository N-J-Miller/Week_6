...
SELECT 
     CONCAT(first_name, ' ', last_name) AS customer --Not strictly necessary, but more elegant.
FROM 
    customer --choosing desired data table
WHERE 
    last_name 
        LIKE 'T%' --WHERE creates a conditional parameter, 
--modified by LIKE, which takes a wild card character, %, to search for data
--that matches the string before the wild card.
ORDER BY 
    first_name;--This is how I sort the output. Default is alphabetically, so there is no
--need to add an extra parameter to do so.

2. Show all rentals returned from 5/28/2005 to 6/1/2005
SELECT 
    * --asterisk selects all 
FROM 
    rental --choosing desired data table
WHERE 
    return_date 
        BETWEEN '2005-05-28' AND '2005-06-01';
-- Using the WHERE conditional and modifying with BETWEEN, AND
-- to pass inclusive dates, as strings and in ISO format, to get a date
-- range. 
...
SELECT 
    film.title, COUNT(rental.inventory_id) AS times_rented
FROM 
    rental
INNER JOIN 
    inventory 
...
/*While not explicity instructed to identify customers by name rather than id, I choose to do so.
This means I will need to link two tables: customer and payment. Then, I will need to sum the amount
each unique customer id paid and SELECT the name associated with that id. To make things tidier, I
will CONCAT last and first names. */
SELECT 
...
SELECT 
	CONCAT(last_name,' ',first_name) AS actor, --organizing data by creating one element from two
	actor.actor_id, COUNT(film_actor.actor_id) AS number_of_roles -- key element that will give my my count value  
 FROM actor
...
/* Does not work; prints all same number. 15861 */
SELECT 
	--rental_duration, 
	--(EXTRACT(DAY FROM return_date) - EXTRACT(DAY FROM rental_date)) AS rental_length,
	--rental_duration - (EXTRACT(DAY FROM return_date) - EXTRACT(DAY FROM rental_date)) AS day_difference,
	COUNT(rental_duration - (EXTRACT(DAY FROM return_date) - EXTRACT(DAY FROM rental_date)) = rental_duration) AS on_time,
	COUNT(rental_duration - (EXTRACT(DAY FROM return_date) - EXTRACT(DAY FROM rental_date)) < rental_duration)AS early,
	COUNT(rental_duration - (EXTRACT(DAY FROM return_date) - EXTRACT(DAY FROM rental_date)) > rental_duration) AS late
FROM film
